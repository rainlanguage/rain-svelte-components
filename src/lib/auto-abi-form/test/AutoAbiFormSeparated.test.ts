import { describe, it, expect } from 'vitest';
import {
    fireEvent,
    render,
    screen,
} from '@testing-library/svelte';
import AutoAbiFormSeparated from '../AutoAbiFormSeparated.svelte';
import OrderBook from './OrderBook.json';
import OrderBookMetadata from './OrderBook.meta.json';
import { get } from 'lodash-es';


function countComponents(component, componentCount) {
    const type: string = component.internalType;

    // Direct components
    if (type == 'string' || type.startsWith('uint') || type == 'address' || type == 'bool' || type == 'bytes') {
        const componentName = `${component.name} (${component.type})`;
        if (componentCount.hasOwnProperty(componentName))
            componentCount[componentName] += 1;
        else {
            componentCount[componentName] = 1;
        }
    }

    // Nested Components 
    if (type.startsWith('struct') && 'components' in component && type !== 'struct EvaluableConfig' && type !== 'struct EvaluableConfig[]') {
        // Recursive call to 
        component.components.forEach(subComponent => {
            componentCount = countComponents(subComponent, componentCount);
        });
    }
    return componentCount;
}

function assertMeta(metadata_, abi_, methodName: string, componentCount_: number, hasMetadataDefined = true) {

    if (hasMetadataDefined) {
        metadata_.methods.forEach(method => {
            // expressions
            if (method.expressions) {
                method.expressions.forEach(expression => {
                    expect(screen.getByText(expression.name)).toBeTruthy();
                    expect(screen.getByText(expression.desc)).toBeTruthy();
                });
            }

            // inputs
            if (method.expressions) {
                method.inputs.forEach(input => {
                    expect(screen.getByText(input.name)).toBeTruthy();
                    expect(screen.getByText(input.desc)).toBeTruthy();

                    get(abi_.abi, input.path).components.forEach((component: { name: string; type: string; }) => {
                        expect(screen.getAllByText(`${component.name} (${component.type})`).length).toEqual(componentCount_);
                    });
                });
            }
        });
    }
    else {
        // Find the method in abi
        const method = abi_.abi.find((method) => method.type == 'function' && method?.name == methodName);

        // Generate component count which will be displayed
        let componentCount = {};
        method.inputs.forEach(component => {
            componentCount = countComponents(component, componentCount);
        });

        // Assert on screen
        for (const [componentName, count] of Object.entries(componentCount)) {
            expect(screen.getAllByText(componentName).length).toEqual(count);
        }

    }
}

describe("AutoAboFormSeparated Tests", () => {
    let addressContext = new Map([]);

    const _deployers = [
        {
            id: '4981eae3-7f4f-56ad-91de-d44e26775d7e',
            address: '0x023be8a728af2e5fccb62ab8159771718043ee1d',
            chainId: 80001,
            created_at: '2023-03-17T02:45:03.625651+00:00',
            opmeta:
                ''
        },
        {
            id: '1b7593ab-9e7e-5a70-90cd-fdcc06390e2f',
            address: '0xac9bba4cde1c29a42ccac3e26e82f794243f1071',
            chainId: 80001,
            created_at: '2023-04-08T04:49:21.184584+00:00',
            opmeta:
                ''
        }
    ];

    beforeAll(async () => {
        addressContext = new Map(
            [
                ['EVALUABLE_ADDRESSES',
                    {
                        getDeployers: async () => {
                            return _deployers;
                        }
                    }
                ]
            ]);
    });

    it("should render the AutoAbiFormComponent for OrderBook's addOrder function", async () => {
        const context = addressContext;
        context.set('onlyExpressionParser', true);

        const { component } = render(AutoAbiFormSeparated, {
            props: {
                abi: OrderBook.abi,
                metadata: OrderBookMetadata,
                methodName: "addOrder"
            },
            context: context
        });

        // Asserting Ui Components
        expect(screen.getByText("Expressions")).toBeTruthy();
        expect(screen.getByText("Configuration")).toBeTruthy();
        expect(screen.getByPlaceholderText("Select interpreter")).toBeTruthy();

        // Asserting meta
        assertMeta(OrderBookMetadata, OrderBook, "addOrder", 2);

        // We need to click something so that the test will await the svelte tick()
        const dummy = await screen.getByText('Order');
        await fireEvent.click(dummy);

        // Asserting result
        const expectedResult = [
            {
                "meta": "",
                "validOutputs": [
                    {
                        "vaultId": "",
                        "decimals": "",
                        "token": ""
                    },
                ],
                "validInputs": [
                    {
                        "vaultId": "",
                        "decimals": "",
                        "token": ""
                    },
                ],
                "evaluableConfig": {
                    "constants": [],
                    "sources": [],
                    "deployer": undefined
                }
            }
        ];

        expect(component.$$.ctx[component.$$.props.result]).toEqual(expectedResult);
    });

    it("should show the simulated output component when onlyExpressionParser context is set to false", async () => {
        const context = addressContext;
        context.set('onlyExpressionParser', false);

        const { component } = render(AutoAbiFormSeparated, {
            props: {
                abi: OrderBook.abi,
                metadata: OrderBookMetadata,
                methodName: "addOrder"
            },
            context: context
        });

        // Asserting Ui Components
        expect(screen.getByText("Expressions")).toBeTruthy();
        expect(screen.getByText("Simulated output")).toBeTruthy();
        expect(screen.getByText("Configuration")).toBeTruthy();
        expect(screen.getByPlaceholderText("Select interpreter")).toBeTruthy();

        // Asserting meta 
        assertMeta(OrderBookMetadata, OrderBook, "addOrder", 2);

        // We need to click something so that the test will await the svelte tick()
        const dummy = await screen.getByText('Order');
        await fireEvent.click(dummy);

        // Asserting result
        const expectedResult = [
            {
                "meta": "",
                "validOutputs": [
                    {
                        "vaultId": "",
                        "decimals": "",
                        "token": ""
                    }
                ],
                "validInputs": [
                    {
                        "vaultId": "",
                        "decimals": "",
                        "token": ""
                    }
                ],
                "evaluableConfig": {
                    "constants": [],
                    "sources": [],
                    "deployer": undefined
                }
            }
        ];
        expect(component.$$.ctx[component.$$.props.result]).toEqual(expectedResult);
    });

    it("should handle and return the result as per user's input", async () => {
        const context = addressContext;
        context.set('onlyExpressionParser', true);
        const inputDecimals = 10e8;
        const inputVaultId = "1337";
        const inputToken = "0x0";

        const { component, container } = render(AutoAbiFormSeparated, {
            props: {
                abi: OrderBook.abi,
                metadata: OrderBookMetadata,
                methodName: "addOrder"
            },
            context: context
        });

        // Asserting meta 
        assertMeta(OrderBookMetadata, OrderBook, "addOrder", 2);

        // Filling Inputs
        const spanElements = container.querySelectorAll("span[slot='label']");
        await spanElements.forEach(async (spanElement) => {
            if (spanElement.textContent?.includes("token")) {
                const input: HTMLInputElement = spanElement.parentElement?.parentElement?.querySelector("input[type=text]")
                expect(input).toBeTruthy();
                await fireEvent.input(input, { target: { value: inputToken } });
            }
            else if (spanElement.textContent?.includes("decimals")) {
                const input: HTMLInputElement = spanElement.parentElement?.parentElement?.querySelector("input[type=number]")
                expect(input).toBeTruthy();
                await fireEvent.input(input, { target: { value: inputDecimals } });
            }
            else if (spanElement.textContent?.includes("vaultId")) {
                const input: HTMLInputElement = spanElement.parentElement?.parentElement?.querySelector("input[type=number]")
                expect(input).toBeTruthy();
                await fireEvent.input(input, { target: { value: inputVaultId } });
            }
        });

        // Asserting result
        const expectedResult = [
            {
                "meta": "",
                "validOutputs": [
                    {
                        "vaultId": `${inputVaultId}`,
                        "decimals": `${inputDecimals}`,
                        "token": `${inputToken}`
                    }
                ],
                "validInputs": [
                    {
                        "vaultId": `${inputVaultId}`,
                        "decimals": `${inputDecimals}`,
                        "token": `${inputToken}`
                    }
                ],
                "evaluableConfig": {
                    "constants": [],
                    "sources": [],
                    "deployer": undefined
                }
            }
        ];

        expect(component.$$.ctx[component.$$.props.result]).toEqual(expectedResult);
    });

    it("should remove the arrayedComponents on button click", async () => {
        const context = addressContext;
        context.set('onlyExpressionParser', true);

        const { component } = render(AutoAbiFormSeparated, {
            props: {
                abi: OrderBook.abi,
                metadata: OrderBookMetadata,
                methodName: "addOrder"
            },
            context: context
        });

        // Asserting Ui Components
        expect(screen.getByText("Expressions")).toBeTruthy();
        expect(screen.getByText("Configuration")).toBeTruthy();
        expect(screen.getByPlaceholderText("Select interpreter")).toBeTruthy();

        // Asserting meta 
        assertMeta(OrderBookMetadata, OrderBook, "addOrder", 2);

        // Removing arrayedComponent
        const removeButtons = screen.getAllByRole('button', {
            name: "Remove"
        });
        await removeButtons.forEach(async (removeButton) => {
            await fireEvent.click(removeButton);
        });

        // Asserting result 
        const expectedResult = [
            {
                "meta": "",
                "validOutputs": [],
                "validInputs": [],
                "evaluableConfig": {
                    "constants": [],
                    "sources": [],
                    "deployer": undefined
                }
            }
        ];
        expect(component.$$.ctx[component.$$.props.result]).toEqual(expectedResult);
    });

    it("should add arrayedComponents on button click", async () => {
        const context = addressContext;
        context.set('onlyExpressionParser', true);

        const { component } = render(AutoAbiFormSeparated, {
            props: {
                abi: OrderBook.abi,
                metadata: OrderBookMetadata,
                methodName: "addOrder"
            },
            context: context
        });

        // Asserting Ui Components
        expect(screen.getByText("Expressions")).toBeTruthy();
        expect(screen.getByText("Configuration")).toBeTruthy();
        expect(screen.getByPlaceholderText("Select interpreter")).toBeTruthy();

        // Asserting meta before adding component
        assertMeta(OrderBookMetadata, OrderBook, "addOrder", 2);

        // Adding arrayedComponent
        const addButtons = screen.getAllByRole('button', {
            name: "+ Add another"
        });

        await addButtons.forEach(async (addButton) => {
            await fireEvent.click(addButton);
        });

        // Asserting meta after adding component
        assertMeta(OrderBookMetadata, OrderBook, "addOrder", 4);

        // Asserting result 
        const expectedResult = [
            {
                "meta": "",
                "validOutputs": [
                    {
                        "vaultId": "",
                        "decimals": "",
                        "token": ""
                    },
                    {
                        "vaultId": "",
                        "decimals": "",
                        "token": ""
                    }
                ],
                "validInputs": [
                    {
                        "vaultId": "",
                        "decimals": "",
                        "token": ""
                    },
                    {
                        "vaultId": "",
                        "decimals": "",
                        "token": ""
                    }
                ],
                "evaluableConfig": {
                    "constants": [],
                    "sources": [],
                    "deployer": undefined
                }
            }
        ];
        expect(component.$$.ctx[component.$$.props.result]).toEqual(expectedResult);
    });

    it("should render the AutoAbiFormComponent for OrderBook's withdraw", async () => {
        const context = addressContext;
        context.set('onlyExpressionParser', true);

        const { component } = render(AutoAbiFormSeparated, {
            props: {
                abi: OrderBook.abi,
                metadata: OrderBookMetadata,
                methodName: "withdraw"
            },
            context: context
        });

        // Asserting Ui Components
        expect(screen.getByText("Configuration")).toBeTruthy();

        // Asserting meta 
        assertMeta(OrderBookMetadata, OrderBook, "withdraw", 0, false);

        // Asserting result
        const expectedResult = [
            {
                amount: '',
                token: '',
                vaultId: '',
            }
        ];
        expect(component.$$.ctx[component.$$.props.result]).toEqual(expectedResult);
    });

    it("should render the AutoAbiFormComponent for OrderBook's removeOrder", async () => {
        const context = addressContext;
        context.set('onlyExpressionParser', true);

        const { component } = render(AutoAbiFormSeparated, {
            props: {
                abi: OrderBook.abi,
                metadata: OrderBookMetadata,
                methodName: "removeOrder"
            },
            context: context
        });

        // Asserting Ui Components
        expect(screen.getByText("Configuration")).toBeTruthy();

        // Asserting meta 
        assertMeta(OrderBookMetadata, OrderBook, "removeOrder", 0, false);

        // We need to click something so that the test will await the svelte tick()
        const dummy = await screen.getByText('Configuration');
        await fireEvent.click(dummy);

        // Asserting result
        const expectedResult = [
            {
                "validOutputs": [
                    {
                        "vaultId": "",
                        "decimals": "",
                        "token": ""
                    }
                ],
                "validInputs": [
                    {
                        "vaultId": "",
                        "decimals": "",
                        "token": ""
                    }
                ],
                "evaluable": {
                    "expression": ""
                },
                "handleIO": false,
                "owner": ""
            }
        ];
        expect(component.$$.ctx[component.$$.props.result]).toEqual(expectedResult);
    });

    it("should render the AutoAbiFormComponent for OrderBook's takeOrders", async () => {
        const context = addressContext;
        context.set('onlyExpressionParser', true);

        const { component } = render(AutoAbiFormSeparated, {
            props: {
                abi: OrderBook.abi,
                metadata: OrderBookMetadata,
                methodName: "takeOrders"
            },
            context: context
        });

        // Asserting Ui Components
        expect(screen.getByText("Configuration")).toBeTruthy();

        // Asserting meta 
        assertMeta(OrderBookMetadata, OrderBook, "takeOrders", 0, false);

        // We need to click something so that the test will await the svelte tick()
        const dummy = await screen.getByText('Configuration');
        await fireEvent.click(dummy);

        // Asserting result
        const expectedResult = [
            {
                "orders": [
                    {
                        "outputIOIndex": "",
                        "inputIOIndex": "",
                        "order": {
                            "validOutputs": [
                                {
                                    "vaultId": "",
                                    "decimals": "",
                                    "token": ""
                                }
                            ],
                            "validInputs": [
                                {
                                    "vaultId": "",
                                    "decimals": "",
                                    "token": ""
                                }
                            ],
                            "evaluable": {
                                "expression": ""
                            },
                            "handleIO": false,
                            "owner": ""
                        },
                        "signedContext": [
                            {
                                "signature": "",
                                "context": [
                                    {}
                                ],
                                "signer": ""
                            }
                        ]
                    }
                ],
                "maximumIORatio": "",
                "maximumInput": "",
                "minimumInput": "",
                "input": "",
                "output": ""
            }
        ];
        expect(component.$$.ctx[component.$$.props.result]).toEqual(expectedResult);
    });

    it("should render the AutoAbiFormComponent for OrderBook's takeOrders", async () => {
        const context = addressContext;
        context.set('onlyExpressionParser', true);

        const { component } = render(AutoAbiFormSeparated, {
            props: {
                abi: OrderBook.abi,
                metadata: OrderBookMetadata,
                methodName: "takeOrders"
            },
            context: context
        });

        // Asserting Ui Components
        expect(screen.getByText("Configuration")).toBeTruthy();

        // Asserting meta 
        assertMeta(OrderBookMetadata, OrderBook, "takeOrders", 0, false);

        // Adding arrayedComponent
        const addButtons = screen.getAllByRole('button', {
            name: "+ Add another"
        });
        // Clicking last add Button to add order element
        await fireEvent.click(addButtons[addButtons.length - 1]);

        // Asserting result
        const expectedResult = [
            {
                "orders": [
                    {
                        "outputIOIndex": "",
                        "inputIOIndex": "",
                        "order": {
                            "validOutputs": [
                                {
                                    "vaultId": "",
                                    "decimals": "",
                                    "token": ""
                                }
                            ],
                            "validInputs": [
                                {
                                    "vaultId": "",
                                    "decimals": "",
                                    "token": ""
                                }
                            ],
                            "evaluable": {
                                "expression": ""
                            },
                            "handleIO": false,
                            "owner": ""
                        },
                        "signedContext": [
                            {
                                "signature": "",
                                "context": [
                                    {}
                                ],
                                "signer": ""
                            }
                        ]
                    },
                    {
                        "outputIOIndex": "",
                        "inputIOIndex": "",
                        "order": {
                            "validOutputs": [
                                {
                                    "vaultId": "",
                                    "decimals": "",
                                    "token": ""
                                }
                            ],
                            "validInputs": [
                                {
                                    "vaultId": "",
                                    "decimals": "",
                                    "token": ""
                                }
                            ],
                            "evaluable": {
                                "expression": ""
                            },
                            "handleIO": false,
                            "owner": ""
                        },
                        "signedContext": [
                            {
                                "signature": "",
                                "context": [
                                    {}
                                ],
                                "signer": ""
                            }
                        ]
                    },
                ],
                "maximumIORatio": "",
                "maximumInput": "",
                "minimumInput": "",
                "input": "",
                "output": ""
            }
        ];

        expect(component.$$.ctx[component.$$.props.result]).toEqual(expectedResult);
    });

    it("should throw an error when EVALUABLE_ADDRESSES context is not defined", async () => {
        const context = addressContext;
        context.set('onlyExpressionParser', true);

        try {
            render(AutoAbiFormSeparated, {
                props: {
                    abi: OrderBook.abi,
                    metadata: OrderBookMetadata,
                    methodName: "addOrder"
                }
            });
        }
        catch (error) {
            expect(error.message).toContain("Context 'EVALUABLE_ADDRESSES' is missing function getDeployers");
        }
    });
});
